...
function Download() {
  ...
  useEffect(async () => {
    if (firstLoad) {
      const res = await axios.get("http://127.0.0.1:80/images/normal");
      ...
      for (var i = 0; i < previewLength; i++) {
        ...
        temp.push("http://127.0.0.1:80/result/normal/" + res.data[i]);
      }
      setImages(temp);
      setFirstLoad(false);
      setFlag(!flag);
  ...
  const downImage = () =>{
      saveAs("http://127.0.0.1/result/normal/result.zip", "image.jpg")
  }
  return (
    <div>
